...
- **Defer Calibre document resolution to org-noter start**: Calibredb-created noter files should be structurally complete and include `CALIBRE_ID`, but leave `NOTER_DOCUMENT` empty until `org-noter` starts. Resolve the real local or downloaded document path from Calibre metadata at that point, then write the concrete path back to `NOTER_DOCUMENT`.
