...
- **Use filenames as Calibre import titles**: Batch Calibre imports must pass an explicit title derived from the source filename. Do not rely on embedded ebook/PDF metadata for title inference, because generic metadata such as `Untitled` can overwrite the meaningful filename.
...
- For Elisp and other interpreted configuration, proposed behavior must be tested in the live runtime first via `emacsclient -e`, a REPL, or another direct temporary eval path. Confirm the behavior works before editing the persistent configuration file; do not use "edit file, then load file" as the first validation path.
- After modifying an Elisp file, reload it via `emacsclient -e '(load-file "path/to/file.el")'` only to verify the persisted file still loads and matches the already-tested behavior.
...
- **Verify loadable code before editing files**: This is a strong rule. Never write Elisp changes directly into config files when the behavior can be checked temporarily. First test the new or changed code in the live runtime via `emacsclient -e` or another direct eval path, then call it with representative inputs and confirm the feature works. Only write the verified code into the config file. Apply the same rule to Python and any other language/runtime with a REPL or direct load mechanism: validate the function and behavior in the runtime first, then edit the production source. If temporary validation is impossible, state why before editing. Emacs keymaps (especially meow's `emulation-mode-map-alists`), `setup` macro expansion, and package loading order can cause runtime behavior to differ significantly from what the source code appears to do.
